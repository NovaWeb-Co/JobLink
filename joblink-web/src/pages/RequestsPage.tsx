...
import { useCreateRequest, useDeleteRequest, useUpdateRequest, useRequests } from "../api/requests.queries";
...
export default function RequestsPage() {
  ...
  const updateMut = useUpdateRequest();
  ...
  function resetForm() {
    setEditingId(null); setDescription(""); setStatus("PENDING");
    setUserId(""); setServiceId("");
  }

  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    const dto = { description, status, userId: Number(userId), serviceId: Number(serviceId) };
    if (editingId !== null) {
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={onSubmit} className="rounded-xl border bg-white p-4 space-y-3">
        <p className="text-sm text-slate-600">
          ...
          {editingId !== null ? "actualiza request y refresca listado." : "crea request y luego invalida cache para refrescar listado."}
        </p>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            <select className="w-full rounded-lg border px-3 py-2" value={status} onChange={(e) => setStatus(e.target.value as RequestStatus)}>
                  ...
                  <td className="p-3">
                    <div className="flex gap-2">
                      <button className="rounded-md border px-2 py-1 hover:bg-slate-50"
                        onClick={() => { setEditingId(r.id); setDescription(r.description ?? ""); setStatus(r.status); setUserId(String(r.userId)); setServiceId(String(r.serviceId)); }}>
                        Editar
                      </button>
                      <button className="rounded-md border px-2 py-1 hover:bg-slate-50 disabled:opacity-50" disabled={deleteMut.isPending}
                        onClick={() => { if (!confirm("¿Borrar esta solicitud?")) return; deleteMut.mutate(r.id); }}>
                        Borrar
                      </button>
                    </div>
                  </td>
