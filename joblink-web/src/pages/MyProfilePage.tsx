...
import {
  ...
  useCreateService, useUpdateService, useDeleteMyService,
...
const CATS = ["Plomería", "Electricidad", "Carpintería", "Pintura", "Limpieza", "Jardinería", "Tecnología", "Transporte", "Salud", "Educación", "Otros"];
...
const CAT_BG: Record<string, string> = {
  ...
  "Salud": "#F0FDF4", "Educación": "#EFF6FF", "Otros": "#F1F5F9"
};
...
export default function MyProfilePage({ openPublish }: { openPublish?: boolean }) {
  ...
  const deleteSvc = useDeleteMyService();
  ...
  // ── Mensajes desde solicitudes ───────────────────────────────────────────
  const [msgToUser, setMsgToUser] = useState<{ reqId: number; userId: number; name: string } | null>(null);
  const [msgContent, setMsgContent] = useState("");

  if (!user) return null;

  useEffect(() => {
    if (openPublish) { setTab("services"); setShowForm(true); }
  }, [openPublish]);
  ...
  const receivedRequests = requests.filter(r => myServices.some(s => s.id === r.serviceId));
  const pendingCount = receivedRequests.filter(r => r.status === "PENDING").length;
  const unreadCount = myMessages.filter(m => !m.isRead && m.receiverId === user.id).length;
  const avgScore = myRatings.length
    ? myRatings.reduce((a, r) => a + r.score, 0) / myRatings.length : null;
  ...
  async function onSubmitProfile(e: React.FormEvent) {
    ...
    await updateUser.mutateAsync({
      id: user!.id,
      dto: {
        name: firstName || undefined, lastname: lastName || undefined,
        email: email || undefined, phone: phone || undefined, address: address || undefined,
      }
    });
    setSavingProfile(false); setProfileSaved(true); setEditProfile(false);
  ...
  async function onSubmitService(e: React.FormEvent) {
    ...
    const dto = {
      ...
      location, availability: avail, userId: user!.id,
      imageUrl: serviceImg || undefined,
    };
    if (editingId !== null) {
      await updateSvc.mutateAsync({ id: editingId, dto });
      if (pendingImgFile) await serviceUpload.uploadFile(pendingImgFile, editingId);
    } else {
      const newService = await createSvc.mutateAsync(dto);
      if (pendingImgFile && newService?.id) await serviceUpload.uploadFile(pendingImgFile, newService.id);
      invalidateServices();
    }
    resetForm();
  }

  // Enviar mensaje a otro usuario (cliente o proveedor)
  async function handleSendMsg(e: React.FormEvent) {
    e.preventDefault();
    if (!msgToUser || !msgContent.trim()) return;
    await createMsg.mutateAsync({ content: msgContent, senderId: user!.id, receiverId: msgToUser.userId });
    setMsgContent(""); setMsgToUser(null);
    setTab("messages");
  }
    ...
    <div className="layout-main" style={{ padding: "2rem 1.25rem" }}>
        ...
        <div style={{ padding: "0 1.75rem 1.75rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "1rem" }}>
            <div style={{ marginTop: -52 }}>
              <ImageUploader currentUrl={currentProfilePhoto} shape="circle" size={92}
                placeholder="Subir foto" uploading={profileUpload.uploading}
                error={profileUpload.error} onUploaded={handleProfilePhoto} />
            </div>
            <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
              ...
              <button className="btn-secondary" onClick={() => setEditProfile(!editProfile)}>✏️ Editar perfil</button>
            </div>
          </div>
          ...
          <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.4rem", margin: "0 0 4px" }}>{user.name} {user.lastname}</h1>
          ...
          {editProfile && (
            <form onSubmit={onSubmitProfile} style={{ marginTop: "1.25rem", paddingTop: "1.25rem", borderTop: "1px solid #F1F5F9", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              ...
              <p style={{ fontSize: "0.78rem", color: "var(--slate)", margin: 0 }}>Para cambiar tu foto, haz clic directamente sobre la imagen de perfil (arriba).</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                <div>
                  <label className="label">Nombre</label>
                  <input className="input" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Ana" />
                </div>
                <div>
                  <label className="label">Apellido</label>
                  <input className="input" value={lastName} onChange={e => setLastName(e.target.value)} placeholder="García" />
                </div>
                <div style={{ gridColumn: "span 2" }}>
                  <label className="label">Email</label>
                  <input type="email" className="input" value={email} onChange={e => setEmail(e.target.value)} placeholder="tu@email.com" />
                  <p style={{ fontSize: "0.7rem", color: "var(--slate)", marginTop: 2 }}>⚠️ Al cambiar el email deberás ingresar con el nuevo correo</p>
                </div>
                <div>
              ...
              <div style={{ display: "flex", gap: "0.75rem", justifyContent: "space-between", flexWrap: "wrap" }}>
                <div style={{ display: "flex", gap: "0.75rem" }}>
                  <button type="submit" className="btn-primary" disabled={savingProfile}>{savingProfile ? "Guardando..." : "Guardar cambios"}</button>
      ...
      {user.role === "USER" && (
        <div className="tab-bar" style={{ marginBottom: "1.5rem" }}>
          ...
          <button className={`tab-btn ${tab === "received" ? "active" : ""}`} onClick={() => setTab("received")} style={{ position: "relative" }}>
            📥 Solicitudes recibidas
            {pendingCount > 0 && (
              <span style={{ position: "absolute", top: 6, right: 6, width: 16, height: 16, borderRadius: "50%", background: "var(--danger)", color: "white", fontSize: "0.6rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {pendingCount}
              </span>
            )}
          </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          {showForm && (
            <div className="card" style={{ padding: "1.5rem" }}>
              <h3 style={{ fontFamily: "Syne, sans-serif", marginBottom: "1.25rem" }}>{editingId ? "Editar servicio" : "Nuevo servicio"}</h3>
              <form onSubmit={onSubmitService} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    <ImageUploader
                      currentUrl={pendingImgFile ? URL.createObjectURL(pendingImgFile) : serviceImg || null}
                      shape="rect" size={100} placeholder="Subir foto"
                      uploading={serviceUpload.uploading} error={serviceUpload.error}
                      onUploaded={(file) => setPendingImgFile(file)} />
                    <div>
                      ...
                      <p style={{ fontSize: "0.75rem", color: "var(--slate)", margin: 0 }}>Formatos: JPG, PNG, WEBP · Máx. 5MB</p>
                    </div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                  <div>
                    ...
                    <input className="input" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ej: Instalaciones eléctricas residenciales" required />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="number" min="0" className="input" value={price} onChange={e => setPrice(e.target.value)} placeholder="Dejar vacío = precio negociable" />
                    <p style={{ fontSize: "0.7rem", color: "var(--slate)", marginTop: 2 }}>💡 Sin precio = el cliente y tú negocian directamente</p>
                  </div>
                  <div>
                    <label className="label">Zona de trabajo</label>
                    <input className="input" value={location} onChange={e => setLocation(e.target.value)} placeholder="Ej: Bogotá, Kennedy y Bosa" />
                  </div>
                  <div style={{ gridColumn: "span 2" }}>
                ...
                <div style={{ display: "flex", gap: "0.75rem" }}>
                  <button type="submit" className="btn-primary" disabled={createSvc.isPending || updateSvc.isPending || serviceUpload.uploading}>
                    {createSvc.isPending || updateSvc.isPending || serviceUpload.uploading ? "Publicando..." : editingId ? "Guardar cambios" : "Publicar servicio"}
                  </button>
                      ...
                      <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                        <button className="btn-ghost" style={{ fontSize: "0.75rem", padding: "4px 10px", flex: 1 }}
                          onClick={() => {
                            ...
                            setCategory(s.category); setPrice(s.price != null ? String(s.price) : "");
                        ...
                        <button className="btn-danger" style={{ flex: 1 }}
                          onClick={() => { if (!confirm("¿Eliminar servicio?")) return; deleteSvc.mutate({ userId: user!.id, serviceId: s.id }); }}>
                          🗑️ Eliminar
                        </button>
                      </div>
                    </div>
      ...
      {/* ── TAB SOLICITUDES ENVIADAS — cliente puede cambiar estado ─────── */}
      {tab === "requests" && (
        myRequests.length === 0 ? (
          <div className="empty">
            <div className="empty-icon">📤</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {myRequests.map(r => {
              // Buscar proveedor del servicio para poder escribirle
              const svc = services.find(s => s.id === r.serviceId);
              return (
                <div key={r.id} className="card" style={{ padding: "1rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                    <div>
                      <p style={{ fontWeight: 600, margin: "0 0 4px", color: "var(--navy)" }}>
                        {r.service ? r.service.title : `Servicio #${r.serviceId}`}
                      </p>
                      {r.description && (
                        <p style={{ fontSize: "0.82rem", color: "var(--slate)", margin: "0 0 6px" }}>"{r.description}"</p>
                      )}
                    ...
                    {/* Acciones del CLIENTE sobre su solicitud */}
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        ...
                        {(r.status === "PENDING" || r.status === "ACCEPTED") && (
                          <button className="btn-danger" style={{ fontSize: "0.8rem" }}
                            disabled={updateReq.isPending}
                            onClick={() => { if (!confirm("¿Cancelar esta solicitud?")) return; updateReq.mutate({ id: r.id, dto: { status: "CANCELED" } }); }}>
                            ✕ Cancelar
                          </button>
                        )}
                      </div>
                      {/* Botón escribir al proveedor */}
                      {svc && svc.userId !== user.id && (
                        <button className="btn-ghost" style={{ fontSize: "0.78rem", padding: "4px 10px" }}
                          onClick={() => {
                            setMsgToUser(msgToUser?.reqId === r.id ? null : { reqId: r.id, userId: svc.userId, name: svc.user?.name ?? "proveedor" });
                            setMsgContent("");
                          }}>
                          💬 {msgToUser?.reqId === r.id ? "Cerrar" : "Escribir al proveedor"}
                        </button>
                      )}
                      {msgToUser?.reqId === r.id && (
                        <form onSubmit={handleSendMsg} style={{ display: "flex", gap: 4, width: "100%" }}>
                          <input className="input" value={msgContent} onChange={e => setMsgContent(e.target.value)}
                            placeholder={`Mensaje a ${msgToUser.name}...`} required style={{ flex: 1, fontSize: "0.82rem" }} />
                          <button type="submit" className="btn-primary" style={{ padding: "4px 10px", fontSize: "0.82rem" }} disabled={createMsg.isPending}>
      ...
      {/* ── TAB SOLICITUDES RECIBIDAS — proveedor puede cambiar estado ──── */}
      {tab === "received" && (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                    <div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                        ...
                        <div>
                          <p style={{ fontWeight: 600, margin: 0, fontSize: "0.875rem", color: "var(--navy)" }}>
                            {client ? `${client.name} ${client.lastname}` : `#${r.userId}`}
                          </p>
                          <p style={{ fontSize: "0.75rem", color: "var(--slate)", margin: 0 }}>
                            Solicita: <b>{svc ? svc.title : `#${r.serviceId}`}</b>
                          </p>
                        </div>
                      </div>
                      {r.description && (
                        <p style={{ fontSize: "0.82rem", color: "var(--slate)", margin: "0 0 6px", fontStyle: "italic" }}>"{r.description}"</p>
                      )}
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span className={`badge ${STATUS_COLORS[r.status]}`}>{STATUS_LABELS[r.status]}</span>
                        <span style={{ fontSize: "0.75rem", color: "var(--slate)" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {r.status === "PENDING" && (
                          <>
                            <button className="btn-ghost" style={{ fontSize: "0.8rem", padding: "6px 12px", color: "var(--success)", border: "1px solid var(--success)" }}
                              disabled={updateReq.isPending}
                        ...
                        {(r.status === "COMPLETED" || r.status === "CANCELED") && (
                          <span style={{ fontSize: "0.78rem", color: "var(--slate)", fontStyle: "italic" }}>
                            {r.status === "COMPLETED" ? "Finalizado" : "Rechazado"}
                          </span>
                        )}
                      </div>
                      {/* Botón escribir al cliente */}
                      {client && (
                        <button className="btn-ghost" style={{ fontSize: "0.78rem", padding: "4px 10px" }}
                          onClick={() => {
                            setMsgToUser(msgToUser?.reqId === r.id ? null : { reqId: r.id, userId: r.userId, name: client.name });
                            setMsgContent("");
                          }}>
                          💬 {msgToUser?.reqId === r.id ? "Cerrar" : "Escribir al cliente"}
                        </button>
                      )}
                      {msgToUser?.reqId === r.id && (
                        <form onSubmit={handleSendMsg} style={{ display: "flex", gap: 4, width: "100%" }}>
      ...
      {tab === "messages" && (() => {
        ...
        myMessages.forEach(m => {
          ...
          if (!conversationMap.has(otherId)) conversationMap.set(otherId, { otherUser, messages: [], unread: 0 });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            {conversations.length === 0 ? (
              <div className="empty">
                ...
                <p style={{ fontSize: "0.85rem" }}>Escríbele a un proveedor desde la página del servicio o desde tus solicitudes.</p>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                {conversations.map(([otherId, conv]) => {
                  ...
                  return (
                    <div key={otherId} className="card" style={{ padding: "1rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "0.875rem" }}>
                        <div style={{ position: "relative", flexShrink: 0 }}>
                          {otherUser?.profilePhoto ? (
                            <img src={toFullUrl(otherUser.profilePhoto) ?? ""} alt=""
                      ...
                      <ReplyForm receiverId={otherId} receiverName={otherUser ? otherUser.name : "usuario"} senderId={user.id} />
                    </div>
                  );
                })}
              </div>
