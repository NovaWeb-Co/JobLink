  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--slate-light)" }}>
      ...
      {page.name === "service-detail" && (
        <ServiceDetailPage
          serviceId={page.id}
          onBack={() => setPage({ name: "services" })}
          onProviderClick={(id) => setPage({ name: "provider", id })}
        />
      )}
      {page.name === "provider" && (
        <ProviderProfilePage
      ...
      {/* Footer */}
      <footer style={{ background: "var(--navy)", marginTop: "4rem", padding: "2rem 0" }}>
        <div className="layout-main" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, background: "var(--gold)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "0.9rem", color: "var(--navy)" }}>JL</div>
            <span style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, color: "white" }}>JobLink</span>
          </div>
          <p style={{ color: "#475569", fontSize: "0.8rem", margin: 0 }}>© 2026 JobLink · Plataforma de servicios locales</p>
        </div>
      </footer>
    </div>
  );
